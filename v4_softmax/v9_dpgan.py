...
def make_discriminator_model():
    ...
    last = layers.Lambda(lambda x: tf.math.l2_normalize(x, axis=1))(x) # L2 normalize embeddings
...
def discriminator_loss(disc_real_output, disc_generated_output):
  return mse(disc_real_output, disc_generated_output)
...
def train(X_train,epochs,init_epoch=1,steps_per_epoch=100,val_steps=10):
  ...
  for epoch in range(init_epoch,epochs+1):
    ...
    for step in range(1,steps_per_epoch+1):
      for target,_ in X_train.take(1):
        gen_loss,disc_loss = train_step(target,epoch)
        if step % 50 == 0:
