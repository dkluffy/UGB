...
def downsample(filters, size, strides=2,apply_batchnorm=True,apply_pool=True):
...
def make_discriminator_model():
    ...
    x = downsample(64, 4, 2,False)(x) 
    x = downsample(128, 2,1)(x) 
    x = downsample(128, 2,1)(x)
    x = downsample(256, 4,2,True)(x)
    x = downsample(256, 4,2,True,False)(x)
    x = downsample(512, 1,1,True,False)(x)
    x = layers.Flatten()(x)
...
discriminator_optimizer = tf.keras.optimizers.SGD(base_learning_rate,0.8)
# lr callback
...
ckpt_manager = tf.train.CheckpointManager(checkpoint, run_logdir+"/v3", max_to_keep=3)

fit_log = os.path.join(run_logdir,"fit_v3",datetime.now().strftime("%Y%m%d-%H%M%S"))
...
for x,y in new_train.take(2):
  ...
  plot_multiple_images(y,4)

train(new_train,X_train,X_val,100,0)
